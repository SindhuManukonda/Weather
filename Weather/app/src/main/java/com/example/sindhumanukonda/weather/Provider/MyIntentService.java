package com.example.sindhumanukonda.weather.provider;
...
import com.example.sindhumanukonda.weather.MyDBHandler;
...
public class MyIntentService extends IntentService {
    ...
    private static String DATA_URL = "http://api.openweathermap.org/data/2.5/group?id=5100619,1261481,2147714,2643741&units=imperial&APPID=fb14773e8e1e835cab2c3af73bcb38ed";
    ...
    public void parseIt(String result) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
                JSONObject productObject = listArray.getJSONObject(i);
                String city = productObject.getString("name");
                JSONObject mainArray = productObject.getJSONObject("main");
                String temp = mainArray.getString("temp")+" F";
                JSONObject windArray = productObject.getJSONObject("wind");
                String speed = windArray.getString("speed")+" mph";
                JSONArray weatherArray = productObject.getJSONArray("weather");
                JSONObject weatherObject = weatherArray.getJSONObject(0);
                String description = weatherObject.getString("description");
                JSONObject sys = productObject.getJSONObject("sys");
...
*/
                ...
                values.put(MyDBHandler.WEATHER_CITY, city);
                values.put(MyDBHandler.WEATHER_TEMP, temp);
                values.put(MyDBHandler.WEATHER_SPEED, speed);
                values.put(MyDBHandler.WEATHER_DESCRIPTION,description);
                values.put(MyDBHandler.WEATHER_COUNTRY,country);
                // values.put(MyDBHandler.WEATHER_DESCRIPTION, description);
    ...
    private void downloadUrl(String myURL) throws IOException {
        ...
        try {
            ...
            getContentResolver().notifyChange(MyContentProvider.CONTENT_URI,
                    null);
